...
#### 功能
- [ ] 新手教學（Onboarding Tour）：逐步高亮核心功能區域，可從設定頁重新觸發
- [ ] 任務小卡拖移排序改寫（Pointer Events 重構，含測試）
- [ ] 跨日偵測修正：today() 改為本地時間；新的一天可設定起始時間（預設 05:00）；設定頁可自訂；本日計劃隨跨日重置
- [ ] 左右滑動動畫 + iOS 質感效果（CSS transition + 分頁指示點）
