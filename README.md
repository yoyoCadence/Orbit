...
**目前版本：** v1.12.0 — [查看版本歷史](CHANGELOG.md)
...
- **設定** — 13 種主題、每日隨機主題、自訂背景圖、任務 CRUD
...
- [x] 撤銷打卡
- [x] 社群排行榜（本週XP + 成長率雙排名）
- [x] 今日頁面任務小卡拖曳排序（bounding-rect 重構，含 reorderTasks 單元測試）
- [x] 「本日計劃」置頂列表（點擊加入，從列表完成才算）
- [x] 跨日偵測（自動彈出起床狀況視窗）
- [x] 跨日偵測修正：today() 改為本地時間；新的一天可設定起始時間（預設 05:00）
- [x] 等級稱號系統（RPG / 鬼滅之刃 / 職場菁英模板 + 自訂稱號）
- [x] 左右滑動手勢切換分頁 + 換頁動畫 + 分頁指示點
- [x] 修正普通模式下預設任務仍可被編輯的問題
- [x] 修正 Skip 確認視窗被計時視窗遮擋（z-index）
- [x] 計時任務略過時可選擇已完成時長（補打卡，上限 3 小時）
- [x] 未達最低時長結束時提示 + 隨機勵志語
- [x] 精力值改為明顯進度條（header 能量條）
...
#### UX / 留存
- [ ] **經驗值曲線重新調整**：前期升等門檻過高，影響新用戶留存；需重新設計 Lv.1~10 的 xpRequired 曲線
- [ ] **新手教學第二步實際操作流程**：引導用戶真正點擊任務加入今日計劃 → 再從計劃卡完成任務；搭配只在教學期間出現的「新手任務卡」（XP=1）
- [ ] **新手教學 spotlight 框顯示不完整**：第 1、2、4 步驟的高亮框被畫面邊緣截斷，需修正捲動或框位計算
- [ ] **任務小卡左滑查看詳細資料**：左滑顯示「詳細」按鈕，點擊開啟任務資訊小視窗（目前有 emoji 點擊版，用戶較不直覺）

#### 功能
- [ ] **設定頁新增每日隨機主題選項**：勾選後每天凌晨自動從已有主題中隨機套用
- [ ] **長按拖曳防止系統選字**：長按任務卡拖曳時 100% 觸發系統文字選取，需用 `user-select: none` + `preventDefault` 解決
- [ ] **本日計劃卡片排序**：計劃區塊新增編輯模式，左側顯示六點拖曳把手，支援拖曳改變計劃任務順序
