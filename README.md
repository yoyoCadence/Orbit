...
- [ ] 今日頁面任務小卡拖曳排序
- [ ] 點擊任務小卡加入「本日計劃」置頂列表，從列表點擊才算開始完成任務
- [ ] 跨日偵測：根據時區判斷是否為新的一天，不重啟 App 也能自動彈出起床狀況視窗
- [ ] 個人頁面等級稱號：修正升級 XP 規則一致性、自訂稱號顯示於排行榜、預設模板（含鬼滅之刃主題）
- [ ] 各頁面支援左右滑動手勢切換
- [ ] 修正普通模式下預設任務仍可被編輯的問題
- [ ] 檢查並調整任務 XP 與前期升級曲線，確保早期升等有吸引力
- [ ] 修正任務進行中選擇 Skip 時，確認視窗被計時視窗遮擋（z-index）的問題
